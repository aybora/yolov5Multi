...
class YoloTime2D_256(nn.Module):
    #This defines the structure of the NN.
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(512, 256, kernel_size=1, padding=0, stride=1)
        ...
        self.bn3 = nn.BatchNorm2d(512, momentum=0.03, eps=1e-3)
        self.bn4 = nn.BatchNorm2d(256, momentum=0.03, eps=1e-3)
        self.bn5 = nn.BatchNorm2d(512, momentum=0.03, eps=1e-3)
        self.bn6 = nn.BatchNorm2d(256, momentum=0.03, eps=1e-3)
...
class YoloTime2D_512(nn.Module):
    #This defines the structure of the NN.
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(1024, 512, kernel_size=1, padding=0, stride=1)
        ...
        self.bn3 = nn.BatchNorm2d(1024, momentum=0.03, eps=1e-3)
        self.bn4 = nn.BatchNorm2d(512, momentum=0.03, eps=1e-3)
        self.bn5 = nn.BatchNorm2d(1024, momentum=0.03, eps=1e-3)
        self.bn6 = nn.BatchNorm2d(512, momentum=0.03, eps=1e-3)
...
class YoloTime2D_1024(nn.Module):
    #This defines the structure of the NN.
    def __init__(self):
        super(YoloTime2D_1024, self).__init__()
        self.conv1 = nn.Conv2d(1024, 2048, kernel_size=(3,9), padding=(1,4), stride=1)
        self.conv2 = nn.Conv2d(2048, 1024, kernel_size=(3,9), padding=(1,4), stride=1)
        self.conv3 = nn.Conv2d(1024, 2048, kernel_size=(3,9), padding=(1,4), stride=(1,3))
        self.conv4 = nn.Conv2d(2048, 1024, kernel_size=1, padding=0, stride=1)
        ...
        self.bn3 = nn.BatchNorm2d(2048, momentum=0.03, eps=1e-3)
        self.bn4 = nn.BatchNorm2d(1024, momentum=0.03, eps=1e-3)
        self.bn5 = nn.BatchNorm2d(2048, momentum=0.03, eps=1e-3)
        self.bn6 = nn.BatchNorm2d(1024, momentum=0.03, eps=1e-3)
...
class YoloTime2D(nn.Module):
    ...
    def __init__(self):
        ...
        self.time256 = YoloTime2D_256()
        self.time512 = YoloTime2D_512()
        self.time1024 = YoloTime2D_1024()
        ...
        self.stride=torch.tensor(allStrides)
        #anch=torch.tensor(allAnchors).view(3,3,2)/self.stride.view(-1,1,1)
        anch=[[x/allStrides[0] for x in allAnchors[0]], [x/allStrides[1] for x in allAnchors[1]],[ x/allStrides[2] for x in allAnchors[2] ]]
        self.yolo=Detect(nc=1, anchors=(anch),ch=(allChannels))
        
    def forward(self, x, targets=None):
        
        x256 = x[:,0:256*64*240].reshape(-1,256,64,240)
        x512 = x[:,256*64*240:256*64*240+512*32*120].reshape(-1,512,32,120)
        x1024 = x[:,256*64*240+512*32*120:256*64*240+512*32*120+1024*16*60].reshape(-1,1024,16,60)
        
        y = []
        
        y.append(self.time256(x256))
        y.append(self.time512(x512))
        y.append(self.time1024(x1024))
